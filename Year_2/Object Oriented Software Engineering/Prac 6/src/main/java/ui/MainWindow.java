...
import edu.curtin.reminder.model.ReminderObserver;
...
{   
    ...
    public void update()
    {      
        List<Reminder> newRemindersList = controller.getReminders();
        showReminders(newRemindersList);
        autosave(newRemindersList);
    }

    private void autosave(List<Reminder> newRemindersList)
    {   
        FileManager fm = new FileManager();
        try
        {
            fm.write(REMINDER_FILE, newRemindersList); //autosaving.
        }
        catch(IOException e)
        {
            System.err.println("Warning: autosave failed!");
        }
    }
}
